% Plotting for sys 6 x 4 
clc; clear all; close all; 
% Spin w/o impurity
Spin = dlmread('../../12Spin.txt') ;
Ntot = length(Spin) ;  % Total number of spin
% Spin with impurity
Spin_Imp = dlmread('../../12SpinImp1.txt') ; 
Sites = 1:length(Spin) ; 
col = 40 ; % number of colors for colorplot
fac = 0.5 ; % Since we want measurement in unit of hbar/2
cmin = 0.10 ; cmax = 0.10

% 2D data
Nx = 6 ; Ny = 4 ; 
Spin2D_raw = dlmread('../6_4_spin.txt') ;
Spin2D_Imp_raw = dlmread('../6_4_spinImp.txt') ; 

Spin2D = vec_to_matrix(Spin2D_raw, Nx, Ny) ;

Spin2D_Imp = vec_to_matrix(Spin2D_Imp_raw, Nx, Ny) ; 

% Plot dashed line
% x(1:2) = 1.5 ; 
y(1) = -10 ; 
y(2) = 10 ;  


% Plot fig 
subplot(2,2,1)
if sum(Spin(:, 3)) < 10^(-10)
   % Since all spins at each site for pure case is 10^(-16), I am setting them
   % manually to zero since colorplot is giving some error otherwise. 
   Spin(:, 3) = 0 ; 
end
% imagesc(Spin(:,3)')
imshow(fac*Spin(:, 3)')
colormap(jet(col)); 
hold on
for num_dash = 1:length(Spin)-1 % We do not need a dash border for last site
x(1:2) = num_dash + 0.5 ; 
plot(x, y, 'k-', 'LineWidth', 0.1)
end

hold off

% colorbar
caxis([-cmin cmax])
% h2 = colorbar
% set(h2,  'FontName', 'times new roman', 'FontSize', 20)
label(1, 0, 0, '$\mathrm{Site~Index}$','$\mathrm{Site~Index}$','$\mathrm{E_x,q_y)}$');  
set(gca,'XMinorTick','off','YMinorTick','off')
set(gca, 'YTick', [])
set(gca, 'XTick', [1:length(Spin)])
axis on

subplot(2,2,2)
% imagesc(Spin_Imp(:,3)')
imshow(fac*Spin_Imp(:, 3)')
colormap(jet(col)); 
caxis([-cmin-0.02 cmax+0.02])

label(1, 0, 0, '$\mathrm{Site~Index}$','$\mathrm{S_i^z}$','$\mathrm{E_x,q_y)}$');  
set(gca,'XMinorTick','off','YMinorTick','off')
set(gca, 'YTick', [])
set(gca, 'XTick', [1:length(Spin)])
axis on
h2 = colorbar
set(h2,  'FontName', 'times new roman', 'FontSize', 20, 'Location', ...
                    'northoutside')
h2.Position
set(h2, 'Position', [0.3600    0.8239    0.3357    0.0127])

subplot(2,2,3)

if sum(sum(Spin2D)) < 10^(-10)
   % Since all spins at each site for pure case is 10^(-16), I am setting them
   % manually to zero since colorplot is giving some error otherwise. 
   Spin2D(:, :) = 0 ; 
end
imshow(fac*Spin2D)
% imagesc(Spin(:,3)')
colormap(jet(col)); 
hold on
for num_dash = 1:Nx-1 % We do not need a dash border for last site
x(1:2) = num_dash + 0.5 ; 
plot(x, y, 'k-', 'LineWidth', 0.1)
end

x(1) = -10 ; 
x(2) = 10 ;  


for num_dash = 1:Ny-1 % We do not need a dash border for last site
y(1:2) = num_dash + 0.5 ; 
plot(x, y, 'k-', 'LineWidth', 0.1)
end

hold off
% colorbar
caxis([-cmin*fac cmax*fac])

label(1, 1, 0, '$\mathrm{Site~Index}$','$\mathrm{Site~Index}$','$\mathrm{E_x,q_y)}$');  
set(gca,'XMinorTick','off','YMinorTick','off')

set(gca, 'XTick', [1:Nx])
axis on
set(gca,'YDir','normal')

subplot(2,2,4)
imshow(fac*Spin2D_Imp)
colormap(jet(col)); 
caxis([-cmin*fac cmax*fac])
label(1, 0, 0, '$\mathrm{Site~Index}$','$\mathrm{S_i^z}$','$\mathrm{E_x,q_y)}$');  
set(gca,'XMinorTick','off','YMinorTick','off')

set(gca,'YDir','normal')
set(gca, 'XTick', [1:Nx])
axis on
h4 = colorbar
set(h4,  'FontName', 'times new roman', 'FontSize', 20, 'Location', ...
                    'northoutside')
h4.Position
set(h4, 'Position', [0.3600    0.4800    0.3357    0.0127])


fig_layout(1)

% Add panel number
dy = 0.24 ; 
xd = -0.5;   yd = 5 ;   
xc = -8.2 ; yc = yd ;
xb = xd ;    yb = yd + 5.0 ;
xa = xc ;    ya = yb ;    
add_txt(xd, yd, '$\mathrm{(d)}$', 'k')
add_txt(xc, yc, '$\mathrm{(c)}$', 'k')
add_txt(xb, yb, '$\mathrm{(b)}$', 'k')
add_txt(xa, ya, '$\mathrm{(a)}$', 'k')

saveas(gcf, '6_4spinplot_v6.pdf')



