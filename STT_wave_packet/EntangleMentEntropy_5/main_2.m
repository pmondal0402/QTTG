...
% Measure result
spin_num(nn) = N ;
M_1_l(nn) = S1(nn) + Sl(nn) - S_1_l(nn); 
S_hlf(nn) = S_12(nn) ;
end

% Save data
data(:,1) = spin_num ; data(:,2) = S_hlf ; data(:,3) = M_1_l ; 
data(:,4) = S1 ; data(:,5) = Sl ; data(:,6) = S_1_l ;
%data(:,1) = time ; data(:,2) = S1; data(:,3) = Sl; data(:,4) = S1 + Sl - S_1_l;
%data(:,5) = S_1_l; data(:,6) = S_12 ;
% dlmwrite(strcat('IE_.txt'), data) ;
dlmwrite(strcat('IE_v2.txt'), data) ;
