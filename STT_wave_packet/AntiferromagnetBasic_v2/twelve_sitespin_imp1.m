...
% Add impurity at site 2 of magnitude 0.2
B0 = 0.0002 ; 
H_imp = sparse(length(Hh), length(Hh)) ; 
ii = 2 ; 
for ii = 1:Ntot 
   H_imp = H_imp + B0*kron(speye(2^(ii-1)), kron(sigz, speye(2^(Ntot-ii)))) ; 
end
...
dlmwrite('12SpinImp1.txt', Spin) ; 
dlmwrite('12Spin_entropyImp1.txt', S_en) ;
